...
def generateCard(pathParam_card, mH1_input, mH2_input, mH3_input, thetahS_input, thetahX_input, thetaSX_input, vs_input, vx_input):
    ...
    '''
    Creates param_card at location pathParam_card for madgraph with values of the
    ...
    pathParam_card: string
        path to the param_card this function generates
    ...
    thetahS_input: float
        the angle thetahS in TRSM

    thetahX_input: float
        the angle thetahX in TRSM

    thetaSX_input: float
        the angle thetaSX in TRSM

    vs_input: float
        the angle vs in TRSM

    vx_input: float
        the angle vx in TRSM
...
def generateScript(pathOutput, pathMadgraphOutput, pathParam_card, pathLoop_sm_twoscalar,
    ...
    e.g. 
    process = 'p p > iota0 > eta0 h [noborn=QCD]' or
    process = 'p p > eta0 h [noborn=QCD]'.

    pathOutput: string
        the path to the directory where the madgraph script is saved.

    pathMadgraphOutput: string
        the path where the madgraph executable sends its output
        (in the madgraph script the path you give to the directory
        after you enter the desired process Madgraph should generate,
        e.g. 
        `generate p p > iota0 > eta0 h [noborn=QCD]
         output pathMadgraphOutput`).

    pathParam_card: string
        the path to the param_card generated by generateCard.

    pathLoop_sm_twoscalar: string
        the path to the directory containing the TRSM model for madgraph
        (https://gitlab.com/apapaefs/twosinglet).
    ...
    process: string
        the physical process for madgraph to generate events in madgraph
        syntax e.g.
        `p p > iota0 > eta0 h [noborn=QCD]` or
        `p p > eta0 h [noborn=QCD]`

    returns: string
        the path to the madgraph script
    '''
...
def mainExecution(pathOutput, pathTempParam_card, pathLoop_sm_twoscalar, nevents, process, runName):
    ...
    pathOutput: string
        the path to the directory where madgraph sends its output
        (obs! not the full path, only the path to the directory
        containing the madgraph output directory).
    
    pathTempParam_card: string
        the path to the param_card.
    ...
    nevents: int
        number of madgraph events for the process.
    ...
    runName: string
        the run name of the output from Madgraph. The madgraph output directory
        can be found in pathOutput as 
        `outputMadgraph_[name of parent directory of pathOutput]_[runName]`.
        runName helps distinguish if the user decides to do several runs
        and stores it in the same directory,'

    returns: tuple
        returns the cross section and its uncertainty generated by madgraph
        of the process given above.
    '''
    ...
    # that the width of h, eta0 or iota0 is nan and Madgraph will
    # raise an error if the param_card with these
    # settings are run, hence we want to avoid that
    ...
    # regex pattern
    pattern = re.compile('Cross-section :.*')
    ...
    crossSec = float(crossSecString[2])
    crossSecUncert = float(crossSecString[4])
...
if __name__ == '__main__':
    ...
    for i in range(len(df)):
        ...
        # generate p p > iota0 > eta0 h [noborn=QCD]
        crossSec_reson, crossSecUncert_reson = mainExecution(pathOutput, pathTempParam_card, pathLoop_sm_twoscalar, 
                                                 nevents, 'p p > iota0 > eta0 h [noborn=QCD]', f'pp_iota0_eta0h_{i}')
        
        dictCrossSec['pp_iota0_eta0h'].append(crossSec_reson)
        dictCrossSec['pp_iota0_eta0h_uncert'].append(crossSecUncert_reson)
        ...
        if crossSec_reson == -1 and crossSec == -1:
            ratio = 0

        # calculate ratio
        else:
            ratio = crossSec/crossSec_reson
