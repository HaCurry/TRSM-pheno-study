...
if __name__ == '__main__':
    ...
    pp_iota0_eta0h_SnS = [dfMadgraph['x_X_SH_SnS'][i] for i in range(len(dfMadgraph))]
    mgSnS_ratio = [dfMadgraph['pp_iota0_eta0h'][i]/dfMadgraph['x_X_SH_SnS'][i]
                   for i in range(len(dfMadgraph))]
    ...
    path_effects = [pe.withStroke(linewidth=0.0, foreground='white')]
    ...
    zi = scipy.interpolate.griddata((ms, mx), mgSnS_ratio, (msMeshi, mxMeshi),
                                    method='cubic')

    print(f'nanmax: {np.nanmax(zi)} and actual max {np.nanmax(mgSnS_ratio)}')
    ...
    msLow, mxLow, mgSnS_ratioLow = cutter(ms, mx, mgSnS_ratio,
                                          (0, 270), (160, 420))

    im = ax.imshow(zi, origin='lower',
                   vmin=min(mgSnS_ratioLow), vmax=max(mgSnS_ratioLow),
    ...
    for i in range(len(mgSnS_ratioLow)):
        if 80 < msLow[i] and mxLow[i] < 260:
            continue
        else:
            ax.annotate('{:.3f}'.format(mgSnS_ratioLow[i]), (msLow[i], mxLow[i]),
                        textcoords='offset points', xytext=(-3, -2),
                        fontsize=fontsize, rotation=rotation, color=color,
                        path_effects=path_effects,)
                        # bbox=dict(boxstyle="square,pad=0.3",
                        #           fc="white", ec="steelblue", alpha=0.5, lw=0))
    ...
    for i in range(len(mgSnS_ratioLow)):
        if 80 > msLow[i] and mxLow[i] > 260:
            ...
            axins.annotate('{:.3f}'.format(mgSnS_ratioLow[i]), (msLow[i], mxLow[i]),
                           textcoords='offset points', xytext=(-3, -2),
                           fontsize=fontsize, rotation=rotation, color=color,
                           path_effects=path_effects)
    ...
    plt.savefig(os.path.join(pathPlots,
                             'ResonVsNonReson_mgVsSnS_Ratio_lowmass.pdf'))
    plt.close()
    ...
    msMed, mxMed, mgSnS_ratioMed = cutter(ms, mx, mgSnS_ratio,
                                          (0, 525), (420, 620))

    im = ax.imshow(zi, origin='lower',
                   vmin=min(mgSnS_ratioMed), vmax=max(mgSnS_ratioMed),
                   extent=[min(ms), max(ms), min(mx), max(mx)], aspect='auto')
    ...
    for i in range(len(mgSnS_ratioMed)):
        ax.annotate('{:.3f}'.format(mgSnS_ratioMed[i]), (msMed[i], mxMed[i]),
                    textcoords='offset points', xytext=(-3, -2),
                    fontsize=fontsize, rotation=rotation, color=color,
                    path_effects=path_effects)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(pathPlots,
                             'ResonVsNonReson_mgVsSnS_Ratio_mediummass.pdf'))
    plt.close()
    ...
    msHigh, mxHigh, mgSnS_ratioHigh = cutter(ms, mx, mgSnS_ratio,
                                             (0, 525), (620, 1200))

    im = ax.imshow(zi, origin='lower',
                   vmin=min(mgSnS_ratioHigh), vmax=max(mgSnS_ratioHigh),
                   extent=[min(ms), max(ms), min(mx), max(mx)], aspect='auto')
    ...
    for i in range(len(mgSnS_ratioHigh)):
        ax.annotate('{:.3f}'.format(mgSnS_ratioHigh[i]), (msHigh[i], mxHigh[i]),
                    textcoords='offset points', xytext=(-3, -2),
                    fontsize=8, rotation=rotation, color=color,
                    path_effects=path_effects)
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(pathPlots,
                             'ResonVsNonReson_mgVsSnS_Ratio_highmass.pdf'))
    plt.close()
