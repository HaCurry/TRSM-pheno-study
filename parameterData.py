...
def parameterMain(listUserParametersDict, targetDir, scannerSmode, **kwargs):
    ...
    for userParametersDict in listUserParametersDict:
        ...
        dataId = paramDirCreator(userParametersDict, targetDir, **kwargs)
...
if __name__ == '__main__':
    ...
    BP2_dictPointlistAtlas = [{'bfb': 0, 'stu': -1, 
                               'mHa_lb': ms_BP2[i], 'mHa_ub': ms_BP2[i],
                               'mHb_lb': 125.09,    'mHb_ub': 125.09,
                               'mHc_lb': mx_BP2[i], 'mHc_ub': mx_BP2[i],
                               'extra': {'ObservedLimit': 10**(-3) * limit_obs_BP2[i], 'dataId': 'S' + str(ms_BP2[i]) + 'and' + 'X' + str(mx_BP2[i]) } } for i in  range(len(ms_BP2))]
    ...
    BP3_dictPointlistAtlas = [{'bfb': 0, 'stu': -1, 
                               'mHa_lb': 125.09,    'mHa_ub': 125.09,
                               'mHb_lb': ms_BP3[i], 'mHb_ub': ms_BP3[i],
                               'mHc_lb': mx_BP3[i], 'mHc_ub': mx_BP3[i],
                               'extra': {'ObservedLimit': 10**(-3) * limit_obs_BP3[i], 'dataId': 'S' + str(ms_BP3[i]) + 'and' + 'X' + str(mx_BP3[i]) } } for i in  range(len(ms_BP3))]
    ...
    # mProcParameterMain(BP2_dictPointlistAtlas, 'BP2', 'ATLAS2023_BP2_prel_scan', 100, 'scan')
    # mProcParameterMain(BP2_dictPointlistAtlas, 'BP2', 'ATLAS2023_BP2_prel_check', 100, 'check')
    print((BP2_dictPointlistAtlas[0])['extra']['dataId'] )

    parameterMain([BP2_dictPointlistAtlas[0]], 'tjolahopp', 'scan', BP = 'BP3', createDir = False, points = 100)
